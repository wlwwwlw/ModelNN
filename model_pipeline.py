...
POPULATION_FILE = BASE_DIR / "population_NN.xlsx"
RESULTS_FILE = BASE_DIR / "results_NN.xlsx"
DISTANCES_FILE = BASE_DIR / "distances_matrix_NN.xlsx"
...
def run_model_once(params: Dict, population: pd.DataFrame, results: pd.DataFrame, distances: pd.DataFrame):
    """
    Запускает одну конфигурацию модели.

    Важно:
    - Для всех моделей строятся одни и те же признаки, секторные оценки,
      матрица взаимодополняемости и пространственная маска.
    - Классические модели кластеризации работают по признаковому пространству X_eval.
    - SpectralClustering работает по предрассчитанной матрице полицентрической связи A.
    - Все модели оцениваются единой системой метрик, включая прикладные
      пространственно-функциональные показатели.
    """
    model_type = params.get("model_type", "spectral")
    ...
    if model_type == "kmeans":
        ...
        if n_clusters < 2 or n_clusters >= n_objects:
            raise ValueError("Некорректное число кластеров для KMeans.")
        model = KMeans(
            n_clusters=n_clusters,
            n_init=20,
        ...
        if n_clusters < 2 or n_clusters >= n_objects:
            raise ValueError("Некорректное число кластеров для AgglomerativeClustering.")
        ...
        model = OPTICS(
            min_samples=int(params["min_samples"]),
            xi=float(params["xi"]),
            min_cluster_size=float(params["min_cluster_size"]),
        )
        labels = model.fit_predict(X_eval)

    elif model_type == "gmm":
        n_components = int(params["n_components"])
        if n_components < 2 or n_components >= n_objects:
            raise ValueError("Некорректное число компонент для GaussianMixture.")
...
def run_optuna_search(population, results, distances):
    print_header("АДАПТИВНЫЙ ПОДБОР ГИПЕРПАРАМЕТРОВ ДЛЯ НЕСКОЛЬКИХ МОДЕЛЕЙ")
        ...
        try:
            ...
            row = {
                **params,
                "trial": trial_number,
                "score": -1e9,
                "model": params.get("model_type", "failed"),
                "error": str(e),
            }
            all_rows.append(row)
            return -1e9
    ...
    def suggest_common_spatial_params(trial):
        """
        Общие параметры, нужные для построения секторных оценок,
        матрицы взаимодополняемости и пространственной маски.
        Они используются при оценке всех моделей по единой системе метрик.
        """
    ...
    def objective(trial):
        model_type = trial.suggest_categorical(
            "model_type",
            ["kmeans", "agglomerative", "dbscan", "optics", "gmm", "spectral"],
        )

        params = suggest_common_spatial_params(trial)
        params["model_type"] = model_type

        # Модельные гиперпараметры
        if model_type == "kmeans":
            ...
            params["eps"] = trial.suggest_float("eps", 0.30, 3.50)
            params["min_samples"] = trial.suggest_int("min_samples", 2, 8)

        elif model_type == "optics":
            params["min_samples"] = trial.suggest_int("min_samples", 2, 8)
            params["xi"] = trial.suggest_float("xi", 0.02, 0.30)
            params["min_cluster_size"] = trial.suggest_float("min_cluster_size", 0.05, 0.35)
    ...
    if best_params is None or best_pack is None:
        raise RuntimeError("Не удалось подобрать ни одной рабочей модели.")
    ...
    if "model" in results_df.columns:
        best_by_model = results_df[results_df["score"] > -1e8].sort_values("score", ascending=False)
        best_by_model = best_by_model.groupby("model", as_index=False).head(1)
        cols = [c for c in ["model", "trial", "score", "n_clusters", "n_components", "eps", "min_samples", "linkage", "covariance_type", "assign_labels"] if c in best_by_model.columns]
